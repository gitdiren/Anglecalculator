...
#def submit():
   ...
   # print("Hello ",nameinfo, "you chose", selected, "passengers.Your destination is: ", chosen, "and dates are=", choice )
...
def calculate():
    ...
    print("Hello ",nameinfo, "you chose", selected, "passengers.Your destination is: ", chosen, "and dates are=", choice, "total cost is", total)


    cust_file = open("custinfo.txt","a")
    cust_file.write(nameinfo+ str(selected)+ chosen+ choice+ str(total))
    cust_file.close()
...
#submit_button = Button(titleframe, text= "Submit Booking", command=submit).grid(row=10,column=1,sticky="e", padx="35")
calculate_button=Button(titleframe, text="Calculate", command=calculate).grid(row=9, column=1, sticky="e", padx="35")
#button_1 = Button(titleframe, text="Enter no of Passengers", command = myClick).grid(row=7,column=0, sticky="w", padx="20")             
